function [trx,perframedata,info,units] = TrackWingsHelper_GUI(trx,moviefile,bgmodel,isarena,params,varargin)
...
[firstframe,DEBUG,restart,framestrack,perframedata] = ...
  myparse(varargin,...
  'firstframe',1,...
  'debug',0,...
  'restart',false,...
...
if ischar(restart),
  load(out.temp_full,'trackdata','perframedata','t','wingtrxprev');
  trx=trackdata.trx;
...
for t = framestrack(:)',
  ...
  debugdata.nframestracked=nframestracked;
  ...
  [wingtrxcurr,debugdata] = TrackWingsOneFrame_GUI(im,bgmodel,isarena,trxcurr,wingtrxprev,params,XGRID,YGRID,debugdata);
